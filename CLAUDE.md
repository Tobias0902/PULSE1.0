...
Decision #11 (PULSE Responsibility & Routing — the generic cross-module assignment/routing authority) is locked below. It is architecture-only: no Module SDK exists yet to actually build it against (see above), so implementation still awaits the Module SDK and its own module-design pass — see "Open decisions" for what that design pass must still resolve.
...
### Decision #11 — PULSE Responsibility & Routing (LOCKED, 2026-08-15)

**Fundamental model:**

- PULSE Responsibility & Routing is a dedicated, first-party, industry-neutral platform module (per Decision #7) — the single generic cross-module authority for determining who inside a tenant is operationally responsible for an incoming information/action item, and for tracking that item's assignment lifecycle. It composes entirely from mechanisms and primitives Decisions #1–#10 already establish; it introduces no new Core schema.
- Guiding distinction: **Competency is not Responsibility.** Competency describes what a person can do; Responsibility describes what a person currently, operationally owns. Both inform routing; only an active Responsibility (or a configured fallback) ever determines the assignment holder.

Binding principles:

1. **Module establishment (user decision a).** PULSE Responsibility & Routing is established as a dedicated first-party platform module under Decision #7's Module SDK, owning its own PostgreSQL schema (Decision #7 §9). It holds no industry-specific business semantics itself; those are supplied by industry profiles/modules through Decision #7 §8's existing generic registries, consistent with Decision #10's Core-neutrality pattern.
2. **Domain as a hierarchical taxonomy mechanism (user decision c).** The generic mechanism for a hierarchical Domain taxonomy (id, key, label, parent reference, owning module/industry-profile, localized terminology) is owned by the Responsibility & Routing module, not added to Core schema. Per Decision #7 §1/§4, Core is reserved for primitives virtually every installation needs to reason about identity (Person, Organisation, Relationship, Location); a routing/classification taxonomy is not among them. "CORE/platform provides the generic mechanism" is realized as this platform module's generic, content-free taxonomy mechanism — it ships with no built-in domain values. Industry profiles/modules (e.g. PULSE+OT) register concrete domain trees (e.g. `Prosthetics -> Lower limb -> Transfemoral -> Knee joints`) through it. The generic platform layer must never hardcode industry terminology. Registration of domain trees reuses Decision #7 §8's existing **configuration schemas** registry — no new SDK registry category is introduced for taxonomies; hierarchy (a parent reference on the Domain record) is a data-shape concern, not a new SDK mechanism. The concrete hierarchy storage strategy (adjacency list, materialized path, or nested set) is deferred to module design.
3. **Responsibility ownership boundary (user decision d).** Responsibility is a Responsibility & Routing-owned record — not a bare Core `Relationship` — that references Core `Person`/`Organisation` and, where site-scoped, Core `Location` (Decision #10) by ID (Decision #7 §7/§9: no cross-schema foreign keys). It carries routing-specific fields a generic Relationship edge does not comfortably hold: domain reference, site/location scope, precedence/priority, validity period, and active/inactive state. A Responsibility may be scoped to any level of the Domain hierarchy, not only leaf domains — this is what allows a "domain owner" fallback (principle 8) to resolve via an ancestor-level Responsibility. A Responsibility's holder may be a Core `Person` or a Core `Organisation` (e.g. a department/team acting as a queue) — no new "queue" primitive is introduced; Organisation already covers this per Decision #10 §3. This keeps Core's `Relationship` primitive semantically neutral (Decision #10 principle 4) while giving routing the structured, queryable shape it needs.
4. **Competency as a distinct concept (user decision b, and "Additional principle").** Competency is a distinct Responsibility & Routing-owned record, independent of Responsibility, referencing Core `Person` by ID: domain reference, proficiency/level, and optional certification/verification/expiry metadata. A Person may hold a Competency without holding the corresponding Responsibility, and vice versa. Competency informs routing recommendations, substitution/escalation candidate selection, and AI-assisted suggestions (principle 10) — it never by itself determines the assignment holder.
5. **Delegation/representation as a distinct concept (user decision e).** Delegation/representation is its own Responsibility & Routing-owned record, distinct from Responsibility: it associates a delegate Person with the Responsibility (or responsible Person) being temporarily represented, carrying an explicit validity period and a reason/source field (e.g. "absence", "planned coverage", "manual override"). It may be informed by absence data sourced from PULSEHuman via the Module SDK (Decision #7 §7), but must never be implicitly derived from absence data alone — an explicit Delegation record is always required for routing purposes, keeping PULSEHuman's absence domain and Responsibility & Routing's eligibility domain cleanly separated (Decision #7 §9).
6. **Assignment independent of Task (user decision f).** Assignment is a Responsibility & Routing-owned lifecycle record (holder, state, history) capable of existing fully independently of any Core Task/Board/Workflow primitive. The module takes no structural dependency on Task, which is not yet implemented (see "Project status"). Once a Core Task primitive exists, an Assignment may optionally project into, or create/link to, a Task via a soft, non-FK origin reference — generalizing Decision #9 §6's pattern from Calendar events to Assignments — without retroactively requiring existing Assignments to migrate.
7. **Daily View projection contract (user decision g).** A generic Daily View projection contract is established as a conceptual requirement: Daily View is presentation/projection only and is never the system of record for the originating information item, the Assignment, or the routing decision. Responsibility & Routing (and any other future source module) publishes into this contract; Daily View must not special-case Responsibility & Routing, or any other source module, by name — generalizing Decision #9 §5's non-special-casing rule from Calendar to Daily View. The contract reuses Decision #7 §8's existing **event subscriptions** registry (source modules publish generic `ActionableItem created/updated/resolved` domain events) and existing **API routes** registry (action invocations — acknowledge/reassign/escalate/complete — are exposed through the owning module's own versioned endpoints) — no new SDK registry category is introduced. Whatever surface ultimately renders Daily View maintains its own lightweight, event-fed projection index rather than querying source modules live, mirroring Calendar's existing projection pattern (Decision #9). User actions taken in Daily View are forwarded back to Responsibility & Routing through this same event/API mechanism, never applied directly to another module's data (Decision #7 §9). Deferred, and explicitly not designed by this decision: the exact event payload schema, and Daily View's own architectural home (a Core mechanism vs. its own module) — a separate future decision.
8. **Unresolved-routing / exception handling (user decision h).** An item that cannot be resolved to an active Responsibility must never be silently dropped. Resolution is attempted through a **configurable, ordered escalation chain**, tenant-defined per domain or per RoutingRule, of the form: primary responsible holder (principle 3) → active substitute/delegate (principle 5) → domain owner (the nearest ancestor Domain node, per principle 3, with an active Responsibility) → configurable central responsibility/management queue (an Organisation-held Responsibility, per principle 3). Each step is attempted in order; a step that itself has no available holder (e.g. the substitute is also absent) falls through to the next. If every configured step fails to resolve, the item enters a **mandatory, non-configurable unresolved-routing/exception queue** that always exists regardless of tenant configuration — this is the guaranteed backstop that makes "never silently dropped" true even under tenant misconfiguration. The exception queue remains fully auditable under the same mechanism as any other Assignment state (principle 11). Deferred: escalation-chain configuration UI and default per-step timing/thresholds.
9. **RoutingRule precedence and tie-break.** Where more than one RoutingRule could resolve an item, precedence is determined deterministically, in this fixed order: (1) **specificity** — a rule matching a deeper/narrower Domain node or narrower context (e.g. domain+site) takes precedence over a more general one; (2) **explicit tenant-configured priority** — an administrator-assigned priority value, used as an override/tiebreaker when specificity alone does not resolve; (3) a **fixed, documented tie-break** (e.g. stable rule identifier/creation order) if still tied, guaranteeing every decision is reproducible and explainable. **AI must never be the tie-break authority for organisational responsibility** — AI plays no role anywhere in this evaluation path (see principle 10); it may only recommend new/changed RoutingRules or a candidate assignment when the deterministic path fails to resolve at all (principle 8's exception queue). Deferred: the exact specificity-scoring formula and the literal tie-break field.
10. **AI-assist boundary and provenance (user decision i).** PULSE AI may, in an advisory capacity only: classify incoming information, estimate relevance, recommend responsible domains/persons, and recommend improvements to RoutingRules. Deterministic, human-approved organisational rules (principle 9) remain the sole authority that actually produces an Assignment; AI must never write an Assignment, alter a RoutingRule, or act as a tie-break authority directly. Every routing decision persists a provenance classification distinguishing at minimum: `rule_derived` (matched an active RoutingRule), `human_decision` (manually assigned/reassigned/overridden), and `ai_recommended_human_approved` (an AI suggestion a human explicitly accepted) — so no routing outcome is ambiguous about whether or how AI was involved.
11. **Auditability.** Every routing decision and every Assignment lifecycle transition (assign/reassign/delegate/escalate/acknowledge/complete) is recorded in the Decision #4 §11 append-only audit log — reusing that existing mechanism rather than introducing a parallel audit system — generalizing Decision #8 §17's connector-attribution pattern to internal routing decisions. The minimal provenance payload per entry carries: the `decisionSource` classification (principle 10), a `ruleRef` (matched RoutingRule and rule-version, when applicable), an `actorRef` (the human who acted/approved, when applicable), and an `aiRecommendationRef` (pointer to the specific AI suggestion shown, when applicable), plus which candidates were considered and the chosen holder. Deferred, and explicitly out of scope for this decision regardless: literal column/table/schema design.
12. **Platform-first sequencing (user decision j).** PULSE Responsibility & Routing is designed and delivered as a standalone platform capability before the Content Request and External Intelligence/Action Request future capabilities are implemented. Both must consume Responsibility & Routing's SDK contract for employee assignment rather than implementing independent routing/assignment logic. This decision does not itself schedule or lock either of those future capabilities — they remain governed by their own recorded-but-not-locked status (see "Recorded future capability" section below).

**Reference — concept ownership:**

| Concept | Ownership | Notes |
|---|---|---|
| Domain | Responsibility & Routing-owned hierarchical taxonomy catalog; values supplied by industry profiles/modules | Not a Core primitive; registered via the existing configuration-schemas registry (principle 2) |
| Responsibility | Responsibility & Routing-owned record referencing Core `Person`/`Organisation`/`Location` by ID | Not a bare `Relationship`; holder may be a Person or an Organisation-held queue; may scope to any Domain level (principle 3) |
| Competency | Responsibility & Routing-owned record referencing Core `Person` by ID | Distinct from Responsibility (principle 4) |
| RoutingRule | Responsibility & Routing-owned, per-tenant configuration | Encodes specificity/priority/tie-break precedence (principle 9) and per-tenant escalation chains (principle 8) |
| Assignment | Responsibility & Routing-owned lifecycle record | Independent of Core `Task` (not yet implemented); may later project into/link to Task (principle 6) |
| Delegation/Representation | Responsibility & Routing-owned record, distinct from Responsibility | Informed by, but never implicitly derived from, PULSEHuman absence data (principle 5) |
| Daily View projection | Conceptual SDK contract requirement, realized via existing event-subscriptions + API-routes registries | Daily View's own architectural home not yet designed (principle 7) |

**Deferred implementation details (explicitly preserved as not-yet-decided; tracked under "Open decisions" below):**

- Daily View: exact `ActionableItem` event payload schema; Daily View's own architectural home (Core mechanism vs. its own module).
- Unresolved-routing escalation chain: configuration UI; default per-step timing/thresholds; whether chains are configured per tenant, per domain, or per RoutingRule.
- RoutingRule precedence: exact specificity-scoring formula; the literal tie-break field.
- Auditability: literal column/table/schema design for the provenance payload (explicitly out of scope regardless, per standing instruction not to create schema).
- Domain taxonomy: hierarchy storage strategy (adjacency list vs. materialized path vs. nested set).

This decision introduces no new Core schema and modifies none of Decisions #1–#10; it composes entirely from mechanisms and primitives they already establish — Decision #7's module/SDK/registry mechanism and event bus, Decision #8's anti-duplication principle and connector-attribution audit pattern, Decision #9's soft-origin-reference and non-special-casing projection pattern, and Decision #10's neutral Person/Organisation/Relationship/Location primitives. It does not reopen or modify Decisions #1–#10.
...
- Daily View's own architectural home (a Core mechanism vs. its own future module) and the exact `ActionableItem` event payload schema for its projection contract (see Decision #11, principle 7)
- Responsibility & Routing's unresolved-routing escalation-chain configuration UI and default per-step timing/thresholds, and whether chains are configured per tenant, per domain, or per RoutingRule (see Decision #11, principle 8)
- Responsibility & Routing's exact RoutingRule specificity-scoring formula and the literal deterministic tie-break field (see Decision #11, principle 9)
- Responsibility & Routing's provenance-payload schema design for routing-decision audit entries (see Decision #11, principle 11) — deferred like all schema/migration work until the Module SDK exists
- Responsibility & Routing's Domain-taxonomy hierarchy storage strategy (adjacency list vs. materialized path vs. nested set) (see Decision #11, principle 2)
